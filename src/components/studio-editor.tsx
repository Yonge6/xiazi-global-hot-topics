...
import { sortPosterFiles } from "@/lib/studio/batch-publish";
...
export function StudioEditor() {
  ...
  const [studioView, setStudioView] = useState<"analytics" | "editor" | "batch">("analytics");
  ...
  const [batchZhFiles, setBatchZhFiles] = useState<File[]>([]);
  const [batchEnFiles, setBatchEnFiles] = useState<File[]>([]);
  ...
  async function loadIssue(entry: IssueEntry) {
    ...
    setBatchDate((current) => current || detail.issue.issueDate);
    setBatchStyle((current) => current || issueStyleName(detail.issue));
  ...
  async function uploadBatchPoster(file: File, locale: "zh" | "en", slug: string, issueDate: string) {
    const form = new FormData();
    form.set("file", file);
    form.set("locale", locale);
    form.set("slug", slug);
    form.set("issueDate", issueDate);
    form.set("isCurrent", "true");
    const response = await fetch("/api/studio/poster", { method: "POST", body: form });
    const detail = await response.json().catch(() => null);
    if (!response.ok) throw new Error(detail?.message || `${locale === "zh" ? "中文" : "英文"}海报上传失败`);
  }

  async function batchPublish() {
    if (!issue) return;
    const zhFiles = sortPosterFiles(batchZhFiles);
    const enFiles = sortPosterFiles(batchEnFiles);
    if (zhFiles.length !== 9 || enFiles.length !== 9) {
      setStatus(`批量发布需要中文 9 张、英文 9 张；当前中文 ${zhFiles.length} 张，英文 ${enFiles.length} 张`);
      return;
    }
    setBatchPublishing(true);
    setStatus("正在解析整段文案…");
    try {
      const previewResponse = await fetch("/api/studio/batch-publish", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mode: "preview",
          issueDate: batchDate,
          copy: batchCopy,
          styleZhName: batchStyle,
          baseIssue: issue,
        }),
      });
      const preview = await previewResponse.json().catch(() => null);
      if (!previewResponse.ok || !preview?.issue) throw new Error(preview?.message || "文案解析失败");

      const nextIssue = preview.issue as Issue;
      for (let index = 0; index < nextIssue.topics.length; index += 1) {
        const topic = nextIssue.topics[index];
        setStatus(`正在上传 NO.${String(index + 1).padStart(2, "0")}：${zhFiles[index].name} / ${enFiles[index].name}`);
        await Promise.all([
          uploadBatchPoster(zhFiles[index], "zh", topic.slug, nextIssue.issueDate),
          uploadBatchPoster(enFiles[index], "en", topic.slug, nextIssue.issueDate),
        ]);
      }

      setStatus("18 张海报已上传，正在发布文字与归档…");
      const publishResponse = await fetch("/api/studio/batch-publish", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode: "publish", issue: nextIssue }),
      });
      const detail = await publishResponse.json().catch(() => null);
      if (!publishResponse.ok) throw new Error(detail?.message || "批量发布失败");

      setIssue(detail.issue);
      setSelectedIssue({ date: detail.issue.issueDate, source: "current", value: "current" });
      setPosterCacheKey(detail.issue.assetVersion || Date.now());
      setBatchCopy("");
      setBatchZhFiles([]);
      setBatchEnFiles([]);
      setStatus(detail.warning ? `批量发布完成；${detail.warning}` : "批量发布完成，文字、海报和归档已写入");
      await loadIssueEntries({ date: detail.issue.issueDate, source: "current", value: "current" });
    } catch (error) {
  ...
  return (
    <main className="studio-shell">
      ...
      <nav className="studio-view-tabs" aria-label="后台功能">
        ...
        <button className={studioView === "batch" ? "active" : ""} onClick={() => setStudioView("batch")}>批量发布</button>
      </nav>
      ...
      {studioView === "analytics" ? (
        ...
        <section className="studio-card studio-batch">
          <div className="studio-order">
            <b>新增一天内容</b>
            <span>{batchZhFiles.length}/9 中文 · {batchEnFiles.length}/9 英文</span>
          </div>
          <label>刊期日期<input type="date" value={batchDate} onChange={(e) => setBatchDate(e.target.value)} /></label>
          <label>主视觉风格<input value={batchStyle} onChange={(e) => setBatchStyle(e.target.value)} placeholder="例如：野兽派全球新闻海报" /></label>
          <label>一次性粘贴中文 + 英文全部文案<textarea className="studio-batch-copy" value={batchCopy} onChange={(e) => setBatchCopy(e.target.value)} placeholder="粘贴包含 NO.01 到 NO.09 的完整文案" /></label>
          <div className="studio-upload studio-batch-upload">
            <label>选择 9 张中文海报<input aria-label="选择 9 张中文海报" type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={(e) => setBatchZhFiles(sortPosterFiles(Array.from(e.target.files || [])))} /></label>
            <label>选择 9 张英文海报<input aria-label="选择 9 张英文海报" type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={(e) => setBatchEnFiles(sortPosterFiles(Array.from(e.target.files || [])))} /></label>
          </div>
          <div className="studio-batch-files">
            <section><b>中文顺序</b>{batchZhFiles.map((file, index) => <small key={file.name}>{index + 1}. {file.name}</small>)}</section>
            <section><b>英文顺序</b>{batchEnFiles.map((file, index) => <small key={file.name}>{index + 1}. {file.name}</small>)}</section>
          </div>
          <p className="studio-lock">发布前会校验 9 条文案、中文 9 张、英文 9 张，并按这里显示的顺序写入 NO.01-NO.09。</p>
        </section>
      ) : (
      <>
      ...
      </div> : studioView === "batch" ? <div className="studio-publish-bar">
        <p role="status" aria-live="polite">{status}</p>
        <button type="button" className="studio-publish" onClick={batchPublish} disabled={batchPublishing}>
          {batchPublishing ? "正在批量发布…" : "一键发布今日文字和 18 张海报"}
        </button>
      </div> : null}
    </main>
  );
...
function issueStyleName(issue: Issue) {
  const style = (issue as Issue & { style?: { zhName?: string; name?: string } }).style;
  return style?.zhName || style?.name || "";
}
