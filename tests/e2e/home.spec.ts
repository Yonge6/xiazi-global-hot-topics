...
test("previews a replacement poster immediately", async ({ page }) => {
  ...
  await expect(page.getByAltText("中文海报预览")).toHaveAttribute("src", /\/api\/posters\/zh\/.*v=poster-test-version/);
  await expect(page.getByRole("status")).toHaveText("海报已替换，预览和下载已切到新图");
});
