...
create unique index if not exists topics_issue_contract_id_unique_idx
  on public.topics(issue_id, contract_id);

create index if not exists sources_contract_id_idx
  on public.sources(contract_id);

grant select on public.issues, public.topics, public.topic_localizations, public.sources, public.poster_assets
  to anon, authenticated, service_role;

grant select, insert, update, delete on public.issues, public.topics, public.topic_localizations, public.sources, public.poster_assets, public.issue_revisions, public.audit_logs
  to service_role;
