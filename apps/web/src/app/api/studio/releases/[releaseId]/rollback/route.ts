...
import { rollbackFuturePublication } from "@/server/releases/release-service";
...
export async function POST(request: Request, context: { params: Promise<{ releaseId: string }> }) {
  ...
  try {
    ...
    const result = await rollbackFuturePublication(releaseId, await request.json(), releaseApproverId());
    return NextResponse.json({ ok: true, result });
  } catch (error) {
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Release 回滚失败" },
      { status: 409 },
    );
  }
}
