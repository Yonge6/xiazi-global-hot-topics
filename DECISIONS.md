...
9 张演示内容卡片是用于界面和数据验证的编辑样稿，其中包含 1 张总览和 8 件热点，不表示 2026 年 6 月 14 日的实时新闻。真实发布前必须由研究流程替换并核验。
...
海报视觉整体保持欢快、阳光、正向，不以阴郁或恐慌制造注意力。每张中英文海报固定显示 `xiazishuo.com` 和真实二维码；二维码优先指向对应热点的独立 URL，详情页完成前使用当前语言首页的热点锚点。`pluto.hk` 仅作为旧入口保留，不作为新海报默认域名。
